...
!********************************************************************************************
! Modernization for Fortran 2008 compliance and gfortran strict build compatibility
! Updated the rootsfinding module to remove legacy GNU Fortran extensions so it 
! can be compiled cleanly with -std=f2008 and strict debug flags.
! By Siew Hoong Wah, 11/jan/2026
module rootsfinding
    !********************************************************************
    ...
    subroutine root3(b,c,d,&    ! input
        ...
        if(DD .lt. 0.D0)then
            ...
            r1 = cmplx( y1,  0.D0, kind=dp)
            r2 = cmplx( y2,  0.D0, kind=dp)
            r3 = cmplx( y3,  0.D0, kind=dp)
            del=3
        elseif(DD .eq. 0.D0)then
            call sort(y1,y2r,y2r,y1,y2r,y2r)
            r1 = cmplx( y1,  0.D0, kind=dp)
            r2 = cmplx(y2r,  0.D0, kind=dp)
            r3 = cmplx(y2r,  0.D0, kind=dp)
            del=3
        else
            r1 = cmplx( y1,  0.D0, kind=dp)
            r2 = cmplx(y2r, y2i, kind=dp)
            r3 = cmplx(y2r,-y2i, kind=dp)
            del=1
        endif
        return
    ...
    subroutine root4(b,c,d,e,r1,r2,r3,r4,reals)
        ...
        integer, parameter :: dp = kind(1.0d0)
        complex(dp) :: r1, r2, r3, r4, s1, s2, s3, temp(1:4), temp1
        ...
        If(del.eq.3)then
            If(real(s3).ge.0.D0)then
                reals=4
                s1=cmplx(real(sqrt(s1)),0.D0, kind=dp)
                s2=cmplx(real(sqrt(s2)),0.D0, kind=dp)
                s3=cmplx(real(sqrt(s3)),0.D0, kind=dp)
            else
                reals=0
                s1=sqrt(s1)
                s2=sqrt(s2)
            ...
            If(real(s1).ge.0.D0)then
                ...
                s1=cmplx(real(sqrt(s1)),0.D0, kind=dp)
                s2=sqrt(s2)
                s3=cmplx(real(s2),-aimag(s2), kind=dp)
            else
                reals=0
                s1=sqrt(s1)
                s2=sqrt(s2)
    ...
    subroutine ellcubiccomplexs(index_p4,a1,b1,a4,b4,f,g,h,y,x,rff_p,integ,cases)
        ...
        if (x .lt. infinity) then
            if (X1*Y1 .ne. 0.d0) then
                ...
                ! choose an alternate expression or skip rc(P2,Q2)
                Q2 = 0.d0
            endif
        else
            if (Y1 .ne. 0.d0 .and. b1 .ne. 0.d0) then
...
module blcoordinate
    ...
    Subroutine radiustp(f12341,a_spin,lambdaBar,robs,lambda,q,r_tp1,&
        ...
        ! Use pure Kerr quartic R_K(r)=0 for turning points.
        ! The wormhole correction enters only through the radial integral
        ! speed factor sqrt(DeltaK/DeltaWH), NOT through the quartic.
    ...
    SUBROUTINE INTRPART(p,f1234r,f1234t,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
        ...
        If(count_num.eq.1)then
            ...
            ! --- Apply wormhole g_rr correction also for the complex-root branch (no turning points fallback) ---
        ...
        !*****************************************************************************************
            ...
            If(f1234r.eq.f1234r_1.and.f1234t.eq.f1234t_1.and.lambda.eq.lambda_1.and.q.eq.q_1.and.&
                ...
                ! --- Apply wormhole g_rr correction for repeated calls (count_num > 1) ---
                IF (lambdaBar .GT. 1.d-10) THEN
                    CALL INTRPART_WH_CORR(robs, r_coord, r_tp1, r_tp2, t1, t2, cases, reals, indrhorizon, &
                        f1234r, lambda, q, a_spin, lambdaBar, dt_corr, dp_corr, da_corr)
                    timer = timer + dt_corr
    ...
    subroutine metricg(robs,sinobs,muobs,a_spin,lambdaBar,somiga,expnu,exppsi,expmu1,expmu2)
        ...
        two=2.D0
        one=1.D0
        ! Kerr Delta: \Delta_K = r^2 - 2r + a^2
        DeltaK = robs**two - two*robs + a_spin**two
        ! Wormhole Delta: \hat{\Delta} = r^2 - 2(1+lambdaBar^2)r + a^2
        ...
        ! Frame dragging frequency \Omega = 2a r / bigA
        somiga=two*a_spin*robs/bigA
        ! Metric exponentials: exp(nu) = sqrt(sigma*DeltaK/bigA), exp(psi) = sin * sqrt(bigA/sigma)
        expnu=sqrt(sigma*DeltaK/bigA)
        exppsi=sinobs*sqrt(bigA/sigma)
        ! Radial metric component uses DeltaWH
    ...
    Subroutine lambdaq_old(alpha,beta,robs,sinobs,muobs,a_spin,scal,velocity,f1234,lambda,q)
        ...
        epmu22=Sigma
        ! equations (110), (112) in Yang & Wang (2012).  Use DeltaWH in the radial square root and q denominator.
        ...
        !write(*,*)'kk=',f1234(3),dsqrt(DeltaWH)*Sigma/bigA*f1234(4)*robs*scal,f1234(3)*somiga*sinobs
        return
    ...
    Subroutine lambdaq(alpha,beta,robs,sinobs,muobs,a_spin,lambdaBar,scal,velocity,f1234,lambda,q)
        ...
        ! equations (1), (2) in Yang & Wang (2012).
        ! In a Kerr‑like wormhole we separate the Kerr and wormhole Deltas.
        DeltaK = robs**two-two*robs + a_spin**two
        DeltaWH = robs**two-two*robs*(one+lambdaBar**two) + a_spin**two
        Sigma=robs**two+(a_spin*muobs)**two
        bigA=(robs**two+a_spin**two)**two-(a_spin*sinobs)**two*DeltaK
    ...
    Subroutine initialdirection(pr,ptheta,pphi,sinobs,&
        muobs,a_spin,lambdaBar,robs,velocity,lambda,q,f1234)
        !********************************************************************************************
...
module sphericalmotion
    ...
    contains
    !********************************************************************************************
    SUBROUTINE SPHERICALMOTION_BL(p,kp,kt,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
        ...
        !*     REVISIONS: ******************************************
        IMPLICIT NONE
        Double precision phyt,timet,kp,kt,p,sinobs,muobs,a_spin,lambdaBar,lambda,q,mu_tp1,tp2,tmu,&
        ...
        c_phi,c_time,DeltaK,thetamax,thetamax_1,sinmax,mumax,Omega,somiga,&
        ...
        IF(count_num.eq.1)then
            ...
            If(a_spin.eq.zero)then
                timet=zero
                CALL SPINZERO(p,kp,kt,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
                robs,thetamax,phyt,timet,sigmat,mucos,t1,t2)
                count_num=count_num+1
                return
            endif
            ...
            Delta = robs**two-two*robs*(one+lambdaBar**two) + a_spin**two
            ! Kerr Δ_K for the time and azimuth coefficients
            DeltaK = robs**two-two*robs+a_spin**two
            c_phi = a_spin*(robs*(two)-(lambda*a_spin))/DeltaK
            c_time = ((two)*robs**three+(two*a_spin*(a_spin-lambda))*robs)/DeltaK
            index_p5(1)=-1
            index_p5(2)=-2
            index_p5(3)=0
            index_p5(4)=-4
    ...
    !********************************************************************************************
    SUBROUTINE SPINZERO(p,kp,kt,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,&
        ...
        kp_1,kt_1,lambda_1,q_1,sinobs_1,muobs_1,a_spin_1,lambdaBar_1,robs_1,sigmat,AA,BB,&
        ...
        p2_phi,p_mu,lambdaBar,Delta,DeltaK,c_time,c_phi,PI2,mu,thetamax,thetamax_1
        integer :: t1,t2,count_num=1,i,j
        save :: kp_1,kt_1,lambda_1,q_1,sinobs_1,muobs_1,a_spin_1,lambdaBar_1,robs_1,PI1_phi,PI2_phi,&
        PI1_time,PI2_time,PI1_sigma,PI2_sigma,Delta,DeltaK,c_time,c_phi,PI1,PI2,mobseqmtp,&
        ...
        ELSE
            IF(kp_1 .eq. kp.and.kt_1 .eq. kt.and.lambda_1 .eq. lambda.and.q_1 .eq.q.and.&
                ...
                .eq. a_spin.and.lambdaBar_1 .eq. lambdaBar.and.robs_1 .eq. robs .and.thetamax_1.eq.thetamax)THEN
                !*******************************************************************************
                IF(q.gt.zero)THEN
                    !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                    If(kt.gt.zero)then
    ...
    !*****************************************************************************************************
    Subroutine lambdaq_sphericalm(r_sm,a_spin,lambdaBar,lambda,q,theta_min)
        !********************************************************************************************
        ...
        sinthemax,costhemax,cotmax,dtor,AL,AE,Sigma,lambdaBar,Delta,lambda,q,&
        ...
        Delta = r_sm**two-two*r_sm*(one+lambdaBar**two) + a_spin**two
...
module pemfinding
    ...
    contains
    !******************************************************************************
    function pemfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
        ...
        use BLcoordinate
        implicit none
        Double precision pemfind,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
        ...
        rhorizon = (one + lambdaBar**two) + sqrt((one + lambdaBar**two)**two - a_spin**two)
        call radiustp(f1234(1),a_spin,lambdaBar,robs,lambda,q,r_tp1,r_tp2,&
        ...
        case(2)
            tr1=0
            tr2=0
            p1 = r2p(f1234(1),rout,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
            tr1=1
            p2 = r2p(f1234(1),rout,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
            !write(*,*)'p1,p2=',p1,p2
            If(caserange.eq.4)then
                pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
                robs,scal,tr1,tr2,p1,p2,NN,Fp,paras,bisection)
            else
                pemfind=Sectionp(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
        ...
        case(3)
            ...
            tr2=0
            p1=r2p(f1234(1),rout,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
            p2=r2p(f1234(1),rin,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
            If(caserange.eq.4)then
                pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
                robs,scal,tr1,tr2,p1,p2,NN,Fp,paras,bisection)
            else
            ...
            If(pemfind.eq.-one)then
                tr1=1
                p1=r2p(f1234(1),rin,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
                p2=r2p(f1234(1),rout,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
                p2 = p2!+1.D-2
                If(caserange.eq.4)then
                    pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
        ...
        case(5)
            ...
            If(r_tp1.gt.rhorizon)then
                ...
                p2=r2p(f1234(1),rhorizon,lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
            endif
            pemfind=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
            robs,scal,tr1,tr2,p1,p2,NN,Fp,paras,bisection)
        end select
    ...
    !*****************************************************************************************************
    Function Sectionp(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
        ...
        use BLcoordinate
        implicit none
        Double precision Sectionp,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,&
        ...
        If(caserange.eq.1 .or.caserange.eq.3)then
            phya1=phi(p1,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal)
            phya2=phi(p2,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal)
        endif
        If(caserange.eq.1 .or.caserange.eq.2)then
            ...
            If((mu1-muup)*(mu1-mudown).lt.zero.or.(mu2-muup)*(mu2-mudown).lt.zero.or.&
                ...
                If(caserange.eq.1 .or.caserange.eq.3)then
                    If((phya1-phy1)*(phya1-phy2).lt.zero.or.(phya2-phy1)*(phya2-phy2).lt.zero.or.&
                        ...
                        Sectionp=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
                        robs,scal,t1,t2,p1,p2,NN,Fp,paras,bisection)
            ...
            If(caserange.eq.1 .or.caserange.eq.3)then
                If((phya1-phy1)*(phya1-phy2).lt.zero.or.(phya2-phy1)*(phya2-phy2).lt.zero.or.&
                    (phy1-phya1)*(phy1-phya2).lt.zero.or.(phy2-phya1)*(phy2-phya2).lt.zero)then
                    Sectionp=rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,p1,p2,NN,Fp,paras,bisection)
                else
    ...
    !*********************************************************************************
    Function rootfind(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
        ...
        use BLcoordinate
        implicit none
        Double precision rootfind,lambda,q,sinobs,muobs,a_spin,lambdaBar,&
        ...
        NNf=floor(NN)
        p=p1
        f_p=Fp(p,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,paras)
        !write(*,*)'NNf=',f_p
        ...
        If(k.le.NNf)then
            ...
            IF(bisection)THEN
                ...
                a_spin,lambdaBar,robs,scal,t1,t2,sp1,sp2,Fp,paras)
            else
    ...
    !*****************************************************************************************************
    Function Bisectionp(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,p1,p2,Fp,paras)
        !*****************************************************************************************************
        ...
        a_spin,lambdaBar,robs,scal,pc,f1,&
        f2,fc,counter,paras(10),f1234(4)
        Double precision,external :: Fp
        integer t1,t2

        pc=(p1+p2)/two
        f1=Fp(p1,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,paras)
        f2=Fp(p2,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,paras)
        fc=Fp(pc,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,paras)
    ...
    !*****************************************************************************************************
    Function NewRapson(f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,p1,p2,Fp,paras)
        !*****************************************************************************************************
        ...
        use BLcoordinate
        implicit none
        Double precision NewRapson,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,&
        ...
        Do k=1,kmax
            ...
            pj=temp+h
            h=pj-temp
            f_p=Fp(temp,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,paras)
            f_pj=Fp(pj,f1234,lambda,q,sinobs,muobs,a_spin,lambdaBar,robs,scal,t1,t2,paras)
            dfp=(f_pj-f_p)/h
            dp=f_p/dfp
            ptn=ptn-dp
...
module obsemitter
    ...
    contains
    !*****************************************************************************************************
    SUBROUTINE alphabetap(a0,B0,alen,Blen,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
        ...
        DOUBLE PRECISION a0,B0,a_spin,lambdaBar,robs,scal,x(n),xem(n),tolx,tolf,sinobs,muobs,a1,B1,&
        ...
        Do i=0,floor(Blen/Bdel)
            ...
            Do j=0,floor(alen/adel)
                ...
                tr1=0
                tr2=0
                call lambdaq(x(1),x(2),robs,sinobs,muobs,a_spin,lambdaBar,scal,obs_V,f1234,lambda,q)
                ! x(3)=r2p(f1234(1),xem(1),lambda,q,a_spin,robs,scal,tr1,tr2)
                x(3)=r2p(f1234(1),xem(1),lambda,q,a_spin,lambdaBar,robs,scal,tr1,tr2)
                If(x(3).lt.zero)cycle
                call mnewt2(ntrial,x,n,tolx,tolf,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
    ...
    !*********************************************************************************
    SUBROUTINE mnewt2(ntrial,x,n,tolx,tolf,sinobs,muobs,a_spin,lambdaBar,robs,&
        ...
        sinobs,muobs,a_spin,lambdaBar,robs,scal,obs_V(3)
        DOUBLE PRECISION, EXTERNAL :: func1,func2,func3
        LOGICAL err

        do k=1,ntrial
            call usrfun2(x,n,NP,fvec,fjac,sinobs,muobs,a_spin,lambdaBar,robs,scal,&
    ...
    !*********************************************************************
    SUBROUTINE usrfun2(x,n,NP,fvec,fjac,sinobs,muobs,a_spin,lambdaBar,robs,&
        scal,obs_V,xend,err,func1,func2,func3)
        !*********************************************************************
        ...
        IMPLICIT NONE
        INTEGER  n,NP,i,j
        DOUBLE PRECISION x(n),fjac(NP,NP),fvec(NP),sinobs,muobs,a_spin,lambdaBar,robs,scal,&
        deltax,f(NP),h,EPS,&
